---
layout: page
title: Nous Contacter
link: Contact
order: 2
---

## Parlons de Votre Projet

Une question sur un produit ? Besoin d'un devis ? Notre équipe vous répond rapidement.

---

<div class="contact-grid">
  <div class="contact-methods">
    <h3>📞 Appelez-nous</h3>
    <p><strong><a href="tel:0553245905" class="phone-number">05 53 24 59 05</a></strong></p>
    <p><strong>Horaires d'ouverture :</strong></p>
    <ul>
      <li>Lundi au Jeudi: 8h00 - 12h00 / 13h00 - 17h00</li>
      <li>Vendredi: 8h00 - 12h00 / 13h00 - 16h00</li>
      <li>Samedi : Fermé</li>
      <li>Dimanche : Fermé</li>
    </ul>
    <p>Notre équipe commerciale est à votre écoute pour vous conseiller et vous établir un devis personnalisé.</p>

    <hr>

    <h3>📠 Fax</h3>
    <p><strong>05 53 58 36 58</strong></p>
    <p>Pour nous transmettre vos plans et commandes.</p>

    <hr>

    <h3>📧 Email</h3>
    <p><strong>contact@metalperigord.fr</strong></p>
    <p>Réponse rapide.</p>

    <hr>

    <h3>📍 Nous Rendre Visite</h3>
    <p><strong>MÉTAL PÉRIGORD</strong><br>
    Impasse Jean Macé<br>
    24100 Bergerac<br>
    France</p>
    <p><strong>Accès facile</strong> : Parking sur place pour déchargement et chargement.</p>
  </div>

  <div class="contact-map">
    <h3>Notre Localisation</h3>
    <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d5656.5065926100315!2d0.49719199999999997!3d44.857138!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x5dfcaa09b4757569!2sMETAL%20PERIGORD!5e0!3m2!1sfr!2sfr!4v1590510759557!5m2!1sfr!2sfr" width="100%" height="400" frameborder="0" style="border:0;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
  </div>
</div>

---

## 🚚 Zone de Livraison

Notre flotte de **4 camions équipés** assure vos livraisons sur :

- **Dordogne (24)** - Livraison express sur demande
- **Gironde (33) et Lot-et-Garonne (47)** - Livraison régulière
- **Départements limitrophes** - Nous consulter

---

## Demande de Devis

Pour recevoir un devis personnalisé, contactez-nous par téléphone au **05 53 24 59 05** ou par email **contact@metalperigord.fr**.

**Informations à nous communiquer pour un devis rapide :**
- Nature des produits (tubes, tôles, poutrelles, etc.)
- Dimensions et épaisseurs souhaitées
- Quantités nécessaires
- Adresse et accès du point de livraison
- Matériel de manutention disponible pour le déchargement
- Délai souhaité

Notre équipe commerciale vous répondra dans les plus brefs délais avec une offre adaptée à vos besoins.

---

## Pourquoi Choisir Métal Périgord ?

<div class="contact-benefits">
  <div>
    <h3>⚡ Réactivité</h3>
    <p>Réponse rapide à vos demandes. Devis établi sous 24h.</p>
  </div>

  <div>
    <h3>💰 Prix Compétitifs</h3>
    <p>Indépendance = meilleurs tarifs. Conditions adaptées aux professionnels.</p>
  </div>

  <div>
    <h3>📦 Stock Permanent</h3>
    <p>2000 tonnes disponibles immédiatement. Pas d'attente.</p>
  </div>

  <div>
    <h3>🎓 Conseil Expert</h3>
    <p>30 ans d'expérience. Aide au choix des bons produits.</p>
  </div>

  <div>
    <h3>✅ Qualité Garantie</h3>
    <p>Produits certifiés conformes aux normes européennes.</p>
    <p>Suivi du RSE</p>
    <p>Possibilité de produit à la norme MACF</p>
  </div>

  <div>
    <h3>🚛 Livraison Rapide</h3>
    <p>Flotte propre. Service express sur toute l'Aquitaine.</p>
  </div>
</div>

---

<div class="cta-box-large">
  <h2>Prêt à Commander ?</h2>
  <p class="lead">Appelez-nous maintenant ou passez nous voir à notre dépôt de Bergerac.</p>
  <a href="tel:0553245905" class="btn btn-primary btn-lg">📞 05 53 24 59 05</a>
</div>

---

## Vous Êtes un Nouveau Client ?

**Bienvenue !** Nous serons ravis de vous accueillir et de vous faire découvrir notre gamme complète de produits acier.

**Pour votre première commande :**
1. Contactez-nous par téléphone ou venez nous rendre visite
2. Exposez-nous votre projet et vos besoins
3. Nous vous conseillons et établissons un devis
4. Validation et mise en production/préparation
5. Livraison rapide sur votre site

**Un interlocuteur dédié** suivra votre dossier pour vous garantir un service personnalisé et efficace.

---

## Questions Fréquentes

**Proposez-vous la découpe sur mesure ?**
Oui, nous pouvons découper vos produits selon vos plans (découpe plasma, oxycoupage, tronçonnage).

**Quels sont vos délais de livraison ?**
Pour les produits en stock (2000 tonnes disponibles), livraison express possible le jour même ou le lendemain selon votre localisation.

**Acceptez-vous les petites quantités ?**
Absolument ! Du particulier bricoleur au grand chantier, tous les clients sont les bienvenus.

**Puis-je venir récupérer ma commande ?**
Oui, vous pouvez retirer vos produits directement à notre dépôt de Bergerac. Parking et zone de chargement sur place.

**Proposez-vous des conditions pour les professionnels ?**
Oui, nous avons des tarifs et conditions de paiement adaptés aux entreprises et artisans. Contactez notre service commercial.

---

<p class="text-center lead">
<strong>Une question ? Un projet ? Contactez-nous !</strong><br>
Nous sommes là pour vous accompagner.
</p>
